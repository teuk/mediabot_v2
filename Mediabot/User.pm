				...
				if (defined($tArgs[0]) && ($tArgs[0] ne "")) {
					...
					unless ($sth->execute($sChannel,$tArgs[0])) {
						...
						while (my $ref = $sth->fetchrow_hashref()) {
							...
							unless (($publictext =~ /^\s*$/) || ($publictext eq ':)') || ($publictext eq ';)') || ($publictext eq ':p') || ($publictext eq ':d') || ($publictext eq ':D') || ($publictext eq ':o') || ($publictext eq ':O') || ($publictext eq '(:') || ($publictext eq '(;') || ($publictext =~ /lol/i) || ($publictext eq 'xD') || ($publictext eq 'XD') || ($publictext eq 'heh') || ($publictext eq 'hah') || ($publictext eq 'huh') || ($publictext eq 'hih')) {
